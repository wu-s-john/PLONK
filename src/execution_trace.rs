...
#[allow(dead_code)]
pub fn interpret_plonk_node_to_execution_trace_table<F: Field>(node: &PlonkNode<NodeMeta<F>>, node_node_equivalences: Vec<(PlonkNodeId, PlonkNodeId)>) -> ExecutionTraceTable<F> {
...
fn build_permutation_groups<F: Field>(plonk_constraints: &mut PlonkContraints<F>) -> WireCellEquivalences {
...
pub fn count_wire_cell_references<F: Field>(
    ...
    let node_equivalences = plonk_constraints.node_node_equivalences.clone();
...
fn build_execution_trace_table<F: Field>(plonk_constraints: &mut PlonkContraints<F>) -> ExecutionTraceTable<F> {
...
mod tests {
    ...
    use crate::execution_trace::{
        count_wire_cell_references, interpret_plonk_node_to_execution_trace_table_helper, PlonkContraints
    };
    use crate::plonk_circuit::{evaluated_node_factory, EvaluatedPlonk};
    ...
    fn basic_single_row_test() {
        let node_0 = evaluated_node_factory::int(0, 0);
        let node_1 = evaluated_node_factory::int(1, 1);
        let node_2 = evaluated_node_factory::add(node_0, node_1, 2);
        ...
        let mut plonk_constraints = PlonkContraints::<F>::new(evaluated_plonk.node_node_equivalences);
        interpret_plonk_node_to_execution_trace_table_helper::<F>(&evaluated_plonk.root, &mut plonk_constraints);
    ...
    fn multi_operation_test() {
        ...
        // 1 + (2 * 8) + 7
        let node_1 = evaluated_node_factory::int(1, 0);      // Node 0: 1
        let node_2 = evaluated_node_factory::int(2, 1);      // Node 1: 2
        let node_8 = evaluated_node_factory::int(8, 2);      // Node 2: 8
        let node_mul = evaluated_node_factory::mult(node_2, node_8, 3); // Node 3: 2*8
        let node_add1 = evaluated_node_factory::add(node_1, node_mul, 4); // Node 4: 1 + (2*8)
        let node_7 = evaluated_node_factory::int(7, 5);      // Node 5: 7
        let root_node = evaluated_node_factory::add(node_add1, node_7, 6); // Node 6: (1+16) +7
    ...
    fn variable_reuse_test() {
        ...
        let node_2 = evaluated_node_factory::int(2, 0);       // Node 0: 2 (bound value)
        let var_x1 = evaluated_node_factory::var("x", 1, F::from(2u64));    // Node 1: x
        let var_x2 = evaluated_node_factory::var("x", 2, F::from(2u64));    // Node 2: x
        let var_x3 = evaluated_node_factory::var("x", 3, F::from(2u64));    // Node 3: x
        let add1 = evaluated_node_factory::add(var_x1, var_x2, 4); // Node 4: x + x
        let add2 = evaluated_node_factory::add(add1, var_x3, 5);  // Node 5: (x + x) + x
        let let_node = evaluated_node_factory::let_("x", node_2, add2, 6); // Node 6: let binding
        ...
        let expected_counts = HashMap::from([
            (0, 5), (1, 5), (2, 5), (3, 5),  // All x references grouped
            (4, 2), 
            (5, 1), (6, 1) // The let node and the body of the let node should be equivalent and reference the same group of wires.
        ]);
    ...
    fn nested_let_reuse_test() {
        ...
        let node_2 = evaluated_node_factory::int(2, 0);        // Node 0: x = 2
        let node_3 = evaluated_node_factory::int(3, 1);        // Node 1: y = 3
        let var_x = evaluated_node_factory::var("x", 2, F::from(2u64));  // Node 2: x
        let var_y1 = evaluated_node_factory::var("y", 3, F::from(3u64)); // Node 3: y
        let add_xy = evaluated_node_factory::add(var_x, var_y1, 4);     // Node 4: x+y
        let let_z = evaluated_node_factory::let_("z", add_xy, 
            {
                let var_z = evaluated_node_factory::var("z", 5, F::from(5u64)); // Node 5: z
                let var_x2 = evaluated_node_factory::var("x", 6, F::from(2u64)); // Node 6: x
                let var_y2 = evaluated_node_factory::var("y", 7, F::from(3u64)); // Node 7: y
                let add_zx = evaluated_node_factory::add(var_z, var_x2, 8);     // Node 8: z+x
                evaluated_node_factory::add(add_zx, var_y2, 9)                  // Node 9: (z+x)+y
            }, 
            10
        );
        let let_y = evaluated_node_factory::let_("y", node_3, let_z, 11);
        let let_x = evaluated_node_factory::let_("x", node_2, let_y, 12);
        ...
        let expected_counts = HashMap::from([
            (0, 4), (2, 4), (6, 4),    // x references
            (1, 4), (3, 4), (7, 4),    // y references
            (4, 2), (5, 2),            // z references
            (8, 2), (9, 1),            // Intermediate adds
            (10, 1), (11, 1), (12, 1)  // Let nodes
        ]);
        ...
        assert_eq!(counts, expected, "Nested let references not properly constrained");
    }
    ...
    fn let_with_unused_binding_test() {
        ...
        let node_2 = evaluated_node_factory::int(2, 0);       // Node 0: 2 (bound value)
        let add_node = evaluated_node_factory::add(evaluated_node_factory::int(3, 1), evaluated_node_factory::int(5, 2), 3); // Node 2: 2 + 5
        let let_node = evaluated_node_factory::let_("x", node_2.clone(), add_node.clone(), 4); // Node 3: let binding
